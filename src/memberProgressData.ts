...
export function summarizeMembers(rows: CourseDetailRow[]): MemberSummary[] {
  ...
  for (const row of rows) {
    ...
    const member = found.get(row.member) || { name: row.member, sections: 0, courses: 0, reports: 0, independentReports: 0, partialReports: 0, measurements: 0, done: 0, required: 0, courseKeys: new Set<string>() }
    member.sections += 1
