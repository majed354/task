...
export default function MemberProgress({ rows, selected, onSelect }: Props) {
  ...
  const filtered = useMemo(() => members.filter((member) => !selected || member.name === selected).sort((a, b) => sortBy === 'progress' ? (b.done / b.required - a.done / a.required) || a.name.localeCompare(b.name, 'ar') : a.name.localeCompare(b.name, 'ar')), [members, selected, sortBy])
